...
(function() {
	...
	global.DD_MODULES['Doodad.Tools.Xml'] = {
		type: null,
		version: '0b',
		namespaces: ['Parsers'],
		...
		create: function create(root, /*optional*/_options) {
			...
			*/
			...
			__Internal__.parseEntities = function parseEntities(data) {
				if (!types.isError(data)) {
					__Internal__.xmlEntities = data;
				};
			};
			
			xml.loadXmlEntities = function loadXmlEntities(/*optional*/callback) {
				...
				return config.loadFile(path, {async: true, watch: true, configPath: configPath, encoding: 'utf8'}, [__Internal__.parseEntities, callback]);
			};
