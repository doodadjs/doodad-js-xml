	...
	const load = function _load(root, genericOutputCb) {
		...
		class SaxHandler {
			...
			destroy() {
				...
				if (this.parser) {
					this.parser.__deallocMethodInternal(this, null);
					this.parser = null;
				};
			}
		};
		...
		class Parser {
			...
			__deallocMethodInternal(/*optional*/obj, /*optional*/fn) {
				if (this.allocatedMethods) {
					if (obj) {
						const methods = this.allocatedMethods.get(obj);
						if (methods) {
							if (fn) {
								...
								this.allocatedMethods.delete(obj);
							};
						};
					} else {
						tools.forEach(this.allocatedMethods, function(methods, cObj) {
							if (fn) {
								const ptr = methods.get(fn);
								if (ptr) {
									clibxml2.removeFunction(ptr);
			...
			destroy() {
				...
				if (this.allocatedMethods) {
					this.__deallocMethodInternal(null, null);
					this.allocatedMethods = null;
				};
