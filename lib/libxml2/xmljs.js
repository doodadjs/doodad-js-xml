	...
	const load = function _load(root, genericOutputCb) {
		...
		const DataRegistry = {
			...
			registerOptions(schemaParserCtxt, options) {
				const xsd = types.get(options, 'xsd', null);
				if (!types.isNothing(xsd)) {
					this.registerBaseDirectory(schemaParserCtxt, xsd);
				};
		...
		const ExternalLoaderHandler = {
			...
			loaderHandler(urlStrPtr, idStrPtr, parserCtxtPtr) {
				...
				let url = files.parseLocation(urlStr, {isFolder: false});
				...
				try {
					...
					if (url.isRelative) {
						...
						if (types.isNothing(content)) {
							try {
								url = libxml2.locateResourceFile(urlStr);
					...
					if (types.isNothing(content)) {
						throw new types.Error("File not found or not relative: " + urlStr);
					};
		...
		class Parser {
			constructor(options) {
				...
				if (xsd) {
					xsd = files.parseLocation(xsd, {isFolder: false});

					if (xsdRoot) {
						const tmp = files.parseLocation(xsdRoot, {isFolder: true});
						if (types._instanceof(tmp, files.Url)) {
							xsd = tmp.combine(xsd);
						} else {
							xsd = files.parsePath(null, {root: tmp.path, path: xsd.path, file: xsd.file});
						};
					};
				...
				if (xsd) {
					this.urlPtr = createStrPtr(xsd.toString());
					if (!this.urlPtr) {
